...
def patch_chromadb():
    ...
    try:
        # ChromaDBのインポートを試行
        import chromadb
        print("ChromaDB正常インポート")
        return True
    except Exception as e:
        if "sqlite3" in str(e):
            print("ChromaDB sqlite3エラー検出、FAISSに置き換え中...")
            
            # 動的にFAISSクラスでChromaを置き換え
            class FakeChroma:
                @staticmethod
                def from_documents(documents, embedding, **kwargs):
                    try:
            ...
            if 'langchain_community.vectorstores' in sys.modules:
                sys.modules['langchain_community.vectorstores'].Chroma = FakeChroma
